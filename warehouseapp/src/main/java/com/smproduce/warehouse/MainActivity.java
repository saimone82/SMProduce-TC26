...
public class MainActivity extends Activity {
    ...
    void buildUi(){
        ...
        Button p=bigButton("PALLETIZATION\nCreate • Scan Cases • Close Pallet",Color.rgb(25,118,210)); p.setOnClickListener(v->openPage("Palletization","/pages/tc26_pallet.php?token="+BuildConfig.TC26_TOKEN)); LinearLayout.LayoutParams pp=new LinearLayout.LayoutParams(-1,160); pp.topMargin=45; home.addView(p,pp);
        Button s=bigButton("SHIPPING\nScan Pallets • PO • Close • BOL",Color.rgb(46,125,50)); s.setOnClickListener(v->openPage("Shipping","/pages/tc26_shipping.php?token="+BuildConfig.TC26_TOKEN)); LinearLayout.LayoutParams sp=new LinearLayout.LayoutParams(-1,160); sp.topMargin=24; home.addView(s,sp);
        TextView hint=new TextView(this); hint.setText("Uses the same pallet & shipping records as the webapp"); hint.setTextColor(Color.rgb(143,169,193)); hint.setTextSize(14); hint.setGravity(Gravity.CENTER); LinearLayout.LayoutParams hp=new LinearLayout.LayoutParams(-1,-2); hp.topMargin=38; home.addView(hint,hp);
        ...
        WebSettings ws=web.getSettings(); ws.setJavaScriptEnabled(true); ws.setDomStorageEnabled(true); ws.setDatabaseEnabled(true); ws.setMixedContentMode(WebSettings.MIXED_CONTENT_NEVER_ALLOW); ws.setUserAgentString(ws.getUserAgentString()+" SMProduceTC26Warehouse/1.2");
